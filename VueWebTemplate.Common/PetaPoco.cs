...
#pragma warning disable 1066,1570,1573,1591
    ...
    public class Database : IDisposable, IDatabase
    {
        ...
        /// <param name="args">Arguments to any embedded parameters in the SQL</param>
        /// <returns>The number of rows affected</returns>
        public int ExecuteOracleProcedure(string sql, DbParameter[] parameters   )
        {
            try
            {
                OpenSharedConnection();
            ...
            try
            {
                ...
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        if (id!=DBNull.Value&&  primaryKeyName != null && !poco.GetType().Name.Contains("AnonymousType"))
                        {
                            PocoColumn pc;
    ...
    public class PocoData
    {
            ...
            else if (!dstType.IsAssignableFrom(srcType))
            {
                ...
                else
                {
                   // return delegate(object src) { return Convert.ChangeType(src, dstType, null); };
                    return delegate(object src) { return ChangeType(src, dstType); };
                }
            }
        ...
        static public object ChangeType(object value, Type type)
        {
            ...
            if (type.IsEnum)
            {
                ...
                {
                    Type ut = type.GetEnumUnderlyingType();
                    if (ut != value.GetType())
                    {
                        var innervalue = ChangeType(value, ut);
                        return Enum.ToObject(type, innervalue);
                    }
            ...
            if (value is int && type == typeof(bool)) return ((int)value > 0);
            if (value is string && type == typeof(bool))
            {
                var valstr = value.ToString().ToLower();
    ...
    public class Sql
    {
        ...
        /// <summary>
        /// 为Like语句转义特殊字符，并在两边添加%
        /// </summary>
        /// <param name="sql"></param>
        /// <param name="args"></param>
        /// <returns></returns>
        public Sql AppendLike(string sql, params object[] args)
        {
            for (var i = 0; i < args.Length; i++)
            {
                var arg = args[i];
                if (arg.GetType() == typeof(String) && args != null)
                {
                    sql = sql.Replace("@" + i, "@" + i + " ESCAPE '\\'");
                    args[i] = "%" + EscapeSQLLike(arg.ToString()) + "%";
                }
            }
            return Append(new Sql(sql, args));
        }
        ...
        /// <summary>
        /// 为Like语句转义特殊字符，并在左边添加%
        /// </summary>
        /// <param name="sql"></param>
        /// <param name="args"></param>
        /// <returns></returns>
        public Sql AppendLikeLeft(string sql, params object[] args)
        {
            for (var i = 0; i < args.Length; i++)
            {
                var arg = args[i];
        ...
        /// <summary>
        /// 为Like语句转义特殊字符，并在右边添加%
        /// </summary>
        /// <param name="sql"></param>
        /// <param name="args"></param>
        /// <returns></returns>
        public Sql AppendLikeRight(string sql, params object[] args)
        {
            for (var i = 0; i < args.Length; i++)
            {
                var arg = args[i];
    ...
    public class OracleDatabaseProvider : DatabaseProvider
    {
        ...
        public override string BuildPageQuery(long skip, long take, SQLParts parts, ref object[] args)
        {
            ...
            var sqlPage = string.Format("select * from ( SELECT rownum peta_rn,page_inner.* FROM (SELECT   {0} ) page_inner  ) peta_paged WHERE peta_rn>@{1} AND peta_rn<=@{2}",
                                  parts.SqlSelectRemoved, args.Length, args.Length + 1);
        ...
        public override DbProviderFactory GetFactory()
        {
            return GetFactory("Oracle.DataAccess.Client.OracleClientFactory, Oracle.DataAccess, Culture=neutral, PublicKeyToken=89b483f429c47342");
        }
